...
public class BallchPlay extends Activity implements BluetoothAdapter.LeScanCallback {
    ...
    private final String[] name = new String[]{"볼 체인지"};
    private final String[] phone = new String[]{"1"};

    public int[] ans = {3, 1, 3, 1, 3, 1, 3, 1}; // 답 스위치 위치
    ArrayList<HashMap<String, String>> personList;
    ListView list;
    private static final String TAG_NAME = "name";
    private static final String TAG_PHONE = "phone";
    private static final int lfront = 1;
    private static final int lback = 2;
    private static final int lfloor = 5;
    private static final int lair = 6;
    private static final int l_vib_both = 15;
    private static final int l_vib_back = 12;
    private static final int l_vib_front = 11;
    public boolean wing = false; //진동이 울린 적이 있는 지 확인하는 변수
    private static final int rfront = 3;
    private static final int rback = 4;
    private static final int rfloor = 7;
    private static final int rair = 8;
    private static final int r_vib_both = 16;
    private static final int r_vib_back = 14;
    private static final int r_vib_front = 13;
    ...
    public void onClickStar1(View v) {
        if (toggle.isChecked()) {
            toggle.setBackgroundDrawable(getResources().
                    getDrawable(R.drawable.starclick));
            Toast.makeText(BallchPlay.this, "즐겨찾기 추가", Toast.LENGTH_SHORT).show();
        } else {
            toggle.setBackgroundDrawable(getResources().
                    getDrawable(R.drawable.staroff));
            Toast.makeText(BallchPlay.this, "즐겨찾기 해제", Toast.LENGTH_SHORT).show();
        }
        try {
            ...
            if (v.getId() == R.id.toggleButton) {
                Cursor c = sampleDB.rawQuery("SELECT * FROM person WHERE name = '볼 체인지'", null);
                ...
                if (c != null && c.getCount() == 0) {
                    ...
                    sampleDB.execSQL("DELETE FROM person WHERE name = '볼 체인지'");
                }
            }
    ...
    protected void showList() {
        ...
        try {
            ...
            Cursor c1 = ReadDB.rawQuery("SELECT * FROM " + tableName + " WHERE name = '볼 체인지'", null);
    ...
    private final Runnable check = new Runnable() {
        ...
        public void run() {
            ...
            if (Device1 != null && Device2 != null) {
                ...
                blechecked = true;
                init();
                Toast.makeText(BallchPlay.this, "연결 성공", Toast.LENGTH_SHORT).show();
                iv.setImageResource(R.drawable.first);
    ...
    private final Runnable moving = new Runnable() {
        ...
        public void run() {
            if (stay == false) {
                Toast.makeText(BallchPlay.this, "연습을 시작해주세요", Toast.LENGTH_SHORT).show();
            }
        }
    };
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            if("TapTap1".equals(gatt.getDevice().getName())){
                ...
                if(newresult){
                  ...
                  /*  if(arrint < 8) {
                        if(data1 != ans[arrint]) { // 현재 순서의 스위치가 아니면


                            switch(data1) { // 각자 진동으로 피드백
                                case lback:
                                    characteristic.setValue(25, BluetoothGattCharacteristic.FORMAT_UINT8, 0);

                                case lfloor:
                                    characteristic.setValue(l_vib_back, BluetoothGattCharacteristic.FORMAT_UINT8, 0);

                                /*case lair:
                                    characteristic.setValue(l_vib_front, BluetoothGattCharacteristic.FORMAT_UINT8, 0);*
                            }
                ...
                if(newresult){
                    ...
                    /*if(arrint < 8) {
                        if(data2 != ans[arrint]) {
                            ...
                            switch(data2) { // 각자 진동으로 피드백
                                case rback:
                                    characteristic.setValue(21, BluetoothGattCharacteristic.FORMAT_UINT8, 0);

                                case rfloor:
                                    characteristic.setValue(r_vib_back, BluetoothGattCharacteristic.FORMAT_UINT8, 0);

                               /* case rair:
                                    characteristic.setValue(r_vib_front, BluetoothGattCharacteristic.FORMAT_UINT8, 0);/
                            }
