...
public class Tap2Play extends Activity implements BluetoothAdapter.LeScanCallback {
    ...
    private final String[] name = new String[]{"drag", "tap1", "탭2"};
    ...
    public void onClickStar1(View v){
        if (toggle.isChecked()){
            toggle.setBackgroundDrawable(getResources().
                    getDrawable(R.drawable.starclick));
            Toast.makeText(Tap2Play.this,"즐겨찾기 추가",Toast.LENGTH_SHORT).show();
        }
        else{
            toggle.setBackgroundDrawable(getResources().
                    getDrawable(R.drawable.staroff));
            Toast.makeText(Tap2Play.this,"즐겨찾기 해제",Toast.LENGTH_SHORT).show();
        }
        try {
            ...
            if (v.getId() == R.id.toggleButton) {
                Cursor c = sampleDB.rawQuery("SELECT * FROM person WHERE name = '탭2'", null);

                if (c != null && c.getCount()==0) {

                    sampleDB.execSQL("INSERT INTO " + tableName
                            + " (name, phone)  Values ('" + name[2] + "', '" + phone[0] + "');");
                }
                else if (c.getCount()==1) {
                    sampleDB.execSQL("DELETE FROM person WHERE name = '탭2'");
                }
            }
    ...
    protected void showList(){
        ...
        try {
            ...
            Cursor c1 = ReadDB.rawQuery("SELECT * FROM " + tableName+ " WHERE name = '탭2'", null);
    ...
    private final Runnable check = new Runnable() {
        ...
        public void run() {
            ...
            if(Device1 != null && Device2 != null) {
                ...
                blechecked=true;
                init();
                Toast.makeText(Tap2Play.this,"연결 성공",Toast.LENGTH_SHORT).show();
                iv.setImageResource(R.drawable.first);
    ...
    private final Runnable moving = new Runnable() {
        ...
        public void run() {
            if(stay==false){
                Toast.makeText(Tap2Play.this,"연습을 시작해주세요",Toast.LENGTH_SHORT).show();
            }
        }
    };
