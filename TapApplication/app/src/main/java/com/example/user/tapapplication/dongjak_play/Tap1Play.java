...
public class Tap1Play extends Activity implements BluetoothAdapter.LeScanCallback {
    ...
    public void onClickStar1(View v){
        ...
        try {
            ...
            if (v.getId() == R.id.toggleButton) {
                Cursor c = sampleDB.rawQuery("SELECT * FROM person WHERE name = '탭1'", null);

                if (c != null && c.getCount()==0) {

                    sampleDB.execSQL("INSERT INTO " + tableName
                            + " (name, phone)  Values ('" + name[1] + "', '" + phone[0] + "');");
                }
                else if (c.getCount()==1) {
                    sampleDB.execSQL("DELETE FROM person WHERE name = '탭1'");
                }
            }
    ...
    protected void showList(){
        ...
        try {
            ...
            Cursor c1 = ReadDB.rawQuery("SELECT * FROM " + tableName+ " WHERE name = '탭1'", null);
